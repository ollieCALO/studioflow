...
function ExportPanel({ campaign, brief }) {
  ...
  const handlePDF = () => {
    ...
    const root = document.createElement('div');
    root.id = 'cpr';
    root.innerHTML = `<h1>${campaign.campaignName}</h1><div class="tag">"${campaign.tagline}"</div><p><strong>Brief:</strong> ${brief}</p><h2>Big Idea & Insight</h2><div class="two"><div><strong>Big Idea</strong><p>${campaign.bigIdea}</p></div><div><strong>Insight</strong><p>${campaign.insight}</p></div></div><h2>Concept</h2><p>${campaign.concept}</p><h2>Campaign Pillars</h2><div class="grid">${[campaign.campaignPillar1,campaign.campaignPillar2,campaign.campaignPillar3].filter(Boolean).map((p,i)=>`<div class="box"><strong>Pillar ${i+1}: ${p.name}</strong><p>${p.description}</p></div>`).join('')}</div><h2>Headlines</h2>${campaign.headlines?.map(h=>`<div class="hl"><strong>${h.headline}</strong><p style="font-size:12px;color:#666;margin:2px 0 0">${h.context}</p></div>`).join('')||''}<h2>Copy Directions</h2>${campaign.copyDirections?.map(c=>`<h3>${c.tone}</h3><p>${c.sample}</p>`).join('')||''}<h2>Visual Direction</h2><p>${campaign.visualDirection?.artDirection}</p><div>${campaign.visualDirection?.moodWords?.map(w=>`<span class="pill">${w}</span>`).join(' ')||''}</div><ul style="margin-top:10px">${campaign.visualDirection?.references?.map(r=>`<li>${r}</li>`).join('')||''}</ul><h2>Channel Strategy</h2>${campaign.channels?.map(ch=>`<h3>${ch.channel}</h3><p>${ch.idea}</p><p style="font-style:italic;color:#555">"${ch.copy}"</p>`).join('')||''}<h2>Creative Guidelines</h2><div class="two"><div><strong style="color:#2d5a3d">✓ Do</strong><ul>${campaign.doList?.map(d=>`<li>${d}</li>`).join('')||''}</ul></div><div><strong style="color:#d4501a">✕ Don't</strong><ul>${campaign.dontList?.map(d=>`<li>${d}</li>`).join('')||''}</ul></div></div><h2>KPIs</h2><ul>${campaign.kpis?.map(k=>`<li>${k}</li>`).join('')||''}</ul><div class="ft">Generated by Studioflow · ${new Date().toLocaleDateString('en-GB',{day:'numeric',month:'long',year:'numeric'})}</div>`;
  ...
  return (
    <div style={{ background: 'var(--warm)', borderRadius: 14, padding: '20px 24px', border: '0.5px solid var(--border)' }}>
      ...
      {status && <div style={{ marginTop: 12, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--accent2)' }}>{status}</div>}
    </div>
  );
}
