...
function ExportPanel({ campaign, brief }) {
  ...
  const handleCopyMarkdown = () => {
    navigator.clipboard?.writeText(buildMarkdown(campaign, brief));
    setCopied(true); setStatus('Copied! Paste into Notion, Google Docs, or anywhere.');
    setTimeout(() => { setCopied(false); setStatus(''); }, 3000);
  };
  ...
  const handlePDF = () => {
    const s = document.createElement('style'); s.id = 'cps';
    s.textContent = '@media print{body>*{display:none!important}#cpr{display:block!important}}#cpr{display:none;font-family:Georgia,serif;max-width:800px;margin:0 auto;padding:40px;color:#111;line-height:1.7}#cpr h1{font-size:34px;margin-bottom:4px}#cpr .tag{font-size:17px;color:#555;font-style:italic;margin-bottom:24px}#cpr h2{font-size:17px;margin-top:28px;border-bottom:1px solid #eee;padding-bottom:4px}#cpr h3{font-size:14px;margin-top:14px}#cpr .pill{display:inline-block;background:#111;color:#fff;padding:2px 10px;border-radius:20px;font-size:11px;margin:2px}#cpr .grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:14px}#cpr .box{border:1px solid #eee;padding:10px;border-radius:6px;font-size:13px}#cpr .two{display:grid;grid-template-columns:1fr 1fr;gap:18px}#cpr ul{padding-left:18px}#cpr li{margin-bottom:3px;font-size:12px}#cpr .hl{border-bottom:1px solid #f0f0f0;padding:8px 0}#cpr .ft{margin-top:36px;font-size:10px;color:#aaa;text-align:center}';
    document.head.appendChild(s);
    const r = document.createElement('div'); r.id = 'cpr';
    r.innerHTML = '<h1>'+campaign.campaignName+'</h1><div class="tag">"'+campaign.tagline+'"</div><p><strong>Brief:</strong> '+brief+'</p><h2>Big Idea</h2><p>'+campaign.bigIdea+'</p><h2>Insight</h2><p>'+campaign.insight+'</p><h2>Concept</h2><p>'+campaign.concept+'</p><h2>Pillars</h2><div class="grid">'+([campaign.campaignPillar1,campaign.campaignPillar2,campaign.campaignPillar3].filter(Boolean).map((p,i)=>'<div class="box"><strong>'+p.name+'</strong><p>'+p.description+'</p></div>').join(''))+'</div><h2>Headlines</h2>'+(campaign.headlines?.map(h=>'<div class="hl"><strong>'+h.headline+'</strong><p style="font-size:11px;color:#666">'+h.context+'</p></div>').join('')||'')+'<h2>Copy</h2>'+(campaign.copyDirections?.map(c=>'<h3>'+c.tone+'</h3><p style="font-size:13px">'+c.sample+'</p>').join('')||'')+'<h2>Visual Direction</h2><p style="font-size:13px">'+(campaign.visualDirection?.artDirection||'')+'</p><div>'+(campaign.visualDirection?.moodWords?.map(w=>'<span class="pill">'+w+'</span>').join(' ')||'')+'</div><h2>Channels</h2>'+(campaign.channels?.map(ch=>'<h3>'+ch.channel+'</h3><p style="font-size:13px">'+ch.idea+'</p><p style="font-size:12px;font-style:italic;color:#555">"'+ch.copy+'"</p>').join('')||'')+'<h2>Guidelines</h2><div class="two"><div><strong style="color:#2d5a3d">Do</strong><ul>'+(campaign.doList?.map(d=>'<li>'+d+'</li>').join('')||'')+'</ul></div><div><strong style="color:#d4501a">Don\'t</strong><ul>'+(campaign.dontList?.map(d=>'<li>'+d+'</li>').join('')||'')+'</ul></div></div><h2>KPIs</h2><ul>'+(campaign.kpis?.map(k=>'<li>'+k+'</li>').join('')||'')+'</ul><div class="ft">Generated by Studioflow</div>';
    document.body.appendChild(r); window.print();
    setTimeout(() => { document.body.removeChild(r); document.head.removeChild(s); }, 1000);
  };

  return (
    <div style={{ background: 'var(--warm)', borderRadius: 14, padding: '20px 24px', border: '0.5px solid var(--border)', marginBottom: 12 }}>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '2px', color: 'var(--muted)', textTransform: 'uppercase', marginBottom: 14 }}>// Export Campaign</div>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        <button onClick={handlePDF} style={{ padding: '10px 18px', borderRadius: 10, border: '0.5px solid var(--border)', background: 'var(--ink)', color: 'var(--paper)', fontSize: 13, fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer' }}>↓ Export as PDF</button>
        <button onClick={handleGoogleDocs} style={{ padding: '10px 18px', borderRadius: 10, border: '0.5px solid var(--border)', background: 'var(--card)', color: 'var(--ink)', fontSize: 13, fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer' }}>↗ Open in Google Docs</button>
        <button onClick={handleCopyMarkdown} style={{ padding: '10px 18px', borderRadius: 10, border: '0.5px solid var(--border)', background: copied ? '#2d5a3d' : 'var(--card)', color: copied ? '#fff' : 'var(--ink)', fontSize: 13, fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s' }}>{copied ? '✓ Copied!' : '⎘ Copy as Markdown'}</button>
      </div>
      {status && <div style={{ marginTop: 12, fontFamily: 'var(--font-mono)', fontSize: 11, color: '#2d5a3d' }}>{status}</div>}
    </div>
  );
}
