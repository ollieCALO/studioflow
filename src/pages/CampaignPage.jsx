...
function buildMarkdown(campaign, brief) {
  const L = [];
  L.push('# ' + campaign.campaignName + '\n> ' + campaign.tagline);
  L.push('\n**Brief:** ' + brief);
  L.push('\n## Big Idea\n' + campaign.bigIdea);
  L.push('\n## Insight\n' + campaign.insight);
  L.push('\n## Concept\n' + campaign.concept);
  (campaign.headlines||[]).forEach(function(h,i){ L.push('\n**'+(i+1)+'. '+h.headline+'**\n_'+h.context+'_'); });
  (campaign.copyDirections||[]).forEach(function(c){ L.push('\n### '+c.tone+'\n'+c.sample); });
  L.push('\n## Visual\n'+((campaign.visualDirection||{}).artDirection||''));
  (campaign.channels||[]).forEach(function(ch){ L.push('\n### '+ch.channel+'\n'+ch.idea+'\n"'+ch.copy+'"'); });
  (campaign.doList||[]).forEach(function(d){ L.push('+ '+d); });
  (campaign.dontList||[]).forEach(function(d){ L.push('- '+d); });
  (campaign.kpis||[]).forEach(function(k){ L.push('* '+k); });
...
function ExportPanel({ campaign, brief }) {
  ...
  const handleCopy = function() {
    navigator.clipboard && navigator.clipboard.writeText(buildMarkdown(campaign, brief));
    setCopied(true); setStatus('Copied to clipboard!');
    setTimeout(function(){ setCopied(false); setStatus(''); }, 2500);
  };
  const handleDocs = function() {
    window.open('https://docs.google.com/document/create', '_blank');
    navigator.clipboard && navigator.clipboard.writeText(buildMarkdown(campaign, brief));
    setStatus('Google Doc opened - paste with Cmd+V');
    setTimeout(function(){ setStatus(''); }, 5000);
  };
  const handlePDF = function() {
    const s = document.createElement('style');
    s.textContent = '@media print{body>*{display:none!important}#cpr{display:block!important}}#cpr{display:none;font-family:Georgia,serif;max-width:760px;margin:0 auto;padding:40px;color:#111;line-height:1.7}#cpr h1{font-size:32px;margin-bottom:6px}#cpr h2{font-size:16px;margin-top:22px;border-bottom:1px solid #eee;padding-bottom:3px}#cpr p,#cpr li{font-size:12px}#cpr ul{padding-left:16px}#cpr .two{display:grid;grid-template-columns:1fr 1fr;gap:16px}';
    document.head.appendChild(s);
    const r = document.createElement('div'); r.id = 'cpr';
    const hl = (campaign.headlines||[]).map(function(h){ return '<p><strong>'+h.headline+'</strong><br><em>'+h.context+'</em></p>'; }).join('');
    const cp = (campaign.copyDirections||[]).map(function(c){ return '<h3>'+c.tone+'</h3><p>'+c.sample+'</p>'; }).join('');
    const ch = (campaign.channels||[]).map(function(c){ return '<h3>'+c.channel+'</h3><p>'+c.idea+'</p><p><em>'+c.copy+'</em></p>'; }).join('');
    const dos = (campaign.doList||[]).map(function(d){ return '<li>'+d+'</li>'; }).join('');
    const dnts = (campaign.dontList||[]).map(function(d){ return '<li>'+d+'</li>'; }).join('');
    const kpis = (campaign.kpis||[]).map(function(k){ return '<li>'+k+'</li>'; }).join('');
    r.innerHTML = '<h1>'+campaign.campaignName+'</h1><p><em>'+campaign.tagline+'</em></p><p><strong>Brief:</strong> '+brief+'</p><h2>Big Idea</h2><p>'+campaign.bigIdea+'</p><h2>Concept</h2><p>'+campaign.concept+'</p><h2>Headlines</h2>'+hl+'<h2>Copy</h2>'+cp+'<h2>Visual Direction</h2><p>'+((campaign.visualDirection||{}).artDirection||'')+'</p><h2>Channels</h2>'+ch+'<h2>Guidelines</h2><div class=\"two\"><div><strong>Do</strong><ul>'+dos+'</ul></div><div><strong>Don\'t</strong><ul>'+dnts+'</ul></div></div><h2>KPIs</h2><ul>'+kpis+'</ul>';
    document.body.appendChild(r); window.print();
    setTimeout(function(){ document.body.removeChild(r); document.head.removeChild(s); }, 1000);
  };
  return (
    <div style={{ background: 'var(--warm)', borderRadius: 14, padding: '20px 24px', border: '0.5px solid var(--border)', marginBottom: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <button onClick={handleDocs} style={{ padding: '10px 18px', borderRadius: 10, border: '0.5px solid var(--border)', background: 'var(--card)', color: 'var(--ink)', fontSize: 13, fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer' }}>&#8599; Open in Google Docs</button>
        <button onClick={handleCopy} style={{ padding: '10px 18px', borderRadius: 10, border: '0.5px solid var(--border)', background: copied ? '#2d5a3d' : 'var(--card)', color: copied ? '#fff' : 'var(--ink)', fontSize: 13, fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer' }}>{copied ? '✓ Copied!' : '⎘ Copy Markdown'}</button>
      </div>
